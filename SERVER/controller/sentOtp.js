...
export async function sendEmail({ recipient_email, OTP }) {
    ...
    transporter.sendMail(mail_configs, function (error, info) {
        if (error) {
            console.error("error for mail sent", error);
            res.status(500).send({ message: `An error has occurred: ${error.message}` });
        } else {
